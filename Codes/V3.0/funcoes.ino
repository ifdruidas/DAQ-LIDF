...
void StartSetupMode() {
  ...
  server.on("/fSensorName5.conf", fSensorName5);
  server.on("/fSensorName6.conf", fSensorName6);
  ...
  server.on("/fSensorIR5.conf", fSensorIR5);
  server.on("/fSensorIR6.conf", fSensorIR6);
  server.on("/Lastbalance", Lastbalance);
  server.on("/Lastdistance", Lastdistance);
  server.on("/Lastgauge", Lastgauge);
...
void fSensorName5() {
  File file = SPIFFS.open("/fSensorName5.conf", "r");
  size_t sent = server.streamFile(file, "text/html");
}
void fSensorName6() {
  File file = SPIFFS.open("/fSensorName6.conf", "r");
  size_t sent = server.streamFile(file, "text/html");
}
void fSensorIR1() {
...
void fSensorIR5() {
  File file = SPIFFS.open("/fSensorIR5.conf", "r");
  size_t sent = server.streamFile(file, "text/html");
}
void fSensorIR6() {
  File file = SPIFFS.open("/fSensorIR6.conf", "r");
  size_t sent = server.streamFile(file, "text/html");
}
void Lastbalance() {
...
void Lastgauge() {
  File file = SPIFFS.open("/Lastgauge", "r");
  size_t sent = server.streamFile(file, "text/html");
}
void fSSID() {
...
void ReadConfig(String FileNameToRead)
{
  ...
  if (FileNameToRead == "fSensorName5.conf") {
    Valor[4] = FileContent;
  }
  if (FileNameToRead == "fSensorName6.conf") {
    Valor[5] = FileContent;
  }
  ...
  if (FileNameToRead == "fSensorIR5.conf") {
    Resp[4] = FileContent;
  }
  if (FileNameToRead == "fSensorIR6.conf") {
    Resp[5] = FileContent;
  }
}
...
void SendHtml() {
  if (client.connect("docs.google.com", 443) == 1)//Tenta se conectar ao servidor do Google docs na porta 443 (HTTPS)
  {
    ...
    band1 = band1/1000;
    //calculo para kg
    Peso = ((Valor[1].toFloat()-Valor[0].toFloat())/(Resp[1].toFloat()-Resp[0].toFloat())*(band1-Resp[0].toFloat()))/1000;
    ...
    Distancia = (Valor[3].toFloat()-Valor[2].toFloat())/(Resp[3].toFloat()-Resp[2].toFloat())*(valor-Resp[2].toFloat());
    toSend += Distancia;
