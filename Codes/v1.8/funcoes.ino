...
void Lastbalance() {
  File file = SPIFFS.open("/Lastbalance.conf", "r");
  size_t sent = server.streamFile(file, "text/html");
}
void Lastdistance() {
  File file = SPIFFS.open("/Lastdistance.conf", "r");
  size_t sent = server.streamFile(file, "text/html");
}
void fSSID() {
...
void SendHtml() {
  if (client.connect("docs.google.com", 443) == 1)//Tenta se conectar ao servidor do Google docs na porta 443 (HTTPS)
  {
    ...
    band1 = scale.read();
    //calculo para kg
    float kg = (Valor[1].toFloat()-Valor[0].toFloat())/(Resp[1].toFloat()-Resp[0].toFloat())*(band1-Valor[0].toFloat());
    Serial.println((String)band1 +" kg: " +(String)kg);
    toSend += kg;//balanca
