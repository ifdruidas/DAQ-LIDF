...
void fileindex()
{
  ...
  if (FilesystemOK == 0) {
    ...
    Serial.println("carreagdo index.html");
...
void filesetup()
{
  Serial.println("carregado setup.html");
...
void savesetup()
{
  ...
  if (server.hasArg("fSensorName1"))
  {
    ...
    //Device Names
    Valor[0]= server.arg("fSensorName1"); Serial.println("Sensor Name1: " + Valor[0]);
    Valor[1] = server.arg("fSensorName2"); Serial.println("Sensor Name2: " + Valor[1]);
    Valor[2] = server.arg("fSensorName3"); Serial.println("Sensor Name3: " + Valor[2]);
    Valor[3] = server.arg("fSensorName4"); Serial.println("Sensor Name4: " + Valor[3]);
    //Device IR Values
    Resp[0] = server.arg("fSensorIR1"); Serial.println("Sensor Val IR1: " + Resp[0]);
    Resp[1] = server.arg("fSensorIR2"); Serial.println("Sensor Val IR2: " + Resp[1]);
    Resp[2] = server.arg("fSensorIR3"); Serial.println("Sensor Val IR3: " + Resp[2]);
    Resp[3] = server.arg("fSensorIR4"); Serial.println("Sensor Val IR4: " + Resp[3]);
    //WIFi
    ...
    //nome dos dipositivo
    WriteFile("fSensorName1.conf", Valor[0]);
    WriteFile("fSensorName2.conf", Valor[1]);
    WriteFile("fSensorName3.conf", Valor[2]);
    WriteFile("fSensorName4.conf", Valor[3]);
    //IR Values
    WriteFile("fSensorIR1.conf", Resp[0]);
    WriteFile("fSensorIR2.conf", Resp[1]);
    WriteFile("fSensorIR3.conf", Resp[2]);
    WriteFile("fSensorIR4.conf", Resp[3]);
    //WIFI
...
for (int i = 0; i <= 3; i++) {
  Serial.println("SensorName1:" + Valor[i] + ":" + Resp[0] );
  Serial.println("SensorName2:" + Valor[1] + ":" + Resp[1] );
  Serial.println("SensorName3:" + Valor[2] + ":" + Resp[2] );
  Serial.println("SensorName4:" + Valor[3] + ":" + Resp[3] );
}
  //Start Wifi
